...
import { createBikeOrder } from '../../api';

export default function BikeRentalScreen() {
  ...
  const [showBikeTypeModal, setShowBikeTypeModal] = useState(false);
  const bikeTypes = ['স্পোর্টস', 'কমিউটার', 'ক্রুজার'];
  ...
  const validateForm = () => {
    ...
    if (!formData.bikeType) newErrors.bikeType = 'বাইকের ধরন নির্বাচন করুন';
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createBikeOrder(formData);
      if (!response) {
  ...
  const renderBikeTypeSelector = () => (
    <Modal
      visible={showBikeTypeModal}
      transparent={true}
      animationType="slide"
      onRequestClose={() => setShowBikeTypeModal(false)}
    >
        ...
        <View style={styles.modalContent}>
          {bikeTypes.map((type) => (
            <TouchableOpacity
              ...
              style={[
                styles.modalOption,
                formData.bikeType === type && styles.selectedModalOption
              ]}
              onPress={() => {
                handleInputChange('bikeType', type);
                setShowBikeTypeModal(false);
              }}
            >
              <Text style={[
                styles.modalOptionText,
                formData.bikeType === type && styles.selectedModalOptionText
              ]}>
                {type}
              </Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            onPress={() => setShowBikeTypeModal(false)}
          >
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.header}>বাইক বুকিং ফর্ম</Text>
      ...
      {/* Bike Type Selector */}
      <View style={styles.formGroup}>
        <Text style={styles.label}>বাইকের ধরন *</Text>
        <TouchableOpacity
          style={[styles.input, styles.bikeTypeInput, errors.bikeType && styles.errorInput]}
          onPress={() => setShowBikeTypeModal(true)}
        >
          <Text style={formData.bikeType ? styles.bikeTypeSelectedText : styles.placeholderText}>
            {formData.bikeType || 'বাইকের ধরন নির্বাচন করুন'}
          </Text>
        </TouchableOpacity>
        {errors.bikeType && <Text style={styles.errorText}>{errors.bikeType}</Text>}
        {renderBikeTypeSelector()}
      </View>
