...
const PharmacyPage = () => {
  ...
  useEffect(() => {
    const fetchPharmacies = async () => {
      try {
        const response = await axios.get('https://api-upokar.onrender.com/getAnimalCare');
        if (response.data && response.data.length > 0) {
          setPharmacies(response.data);
        } else {
          setError('No animal care found');
        }
      } catch (err) {
        setError('Failed to fetch animal care. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
    ...
    if (filteredPharmacies.length === 0) {
      return (
        <View style={styles.centered}>
          ...
          <Text style={styles.noResultsText}>কোন পশু চিকিৎসা পাওয়া যায়নি</Text>
        </View>
      );
    }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TextInput
          ...
          placeholder="পশু চিকিৎসা খুঁজুন..."
