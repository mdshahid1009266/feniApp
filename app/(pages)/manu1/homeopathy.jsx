...
const HomeopathyPage = () => {
  ...
  const handleWhatsApp = (phoneNumber) => {
    Linking.openURL(`whatsapp://send?phone=+88${phoneNumber}&text=হ্যালো%20হোমিওপ্যাথি`);
  };

  const filteredHomeopathies = homeopathies.filter(homeopathy =>
    homeopathy.name.toLowerCase().includes(searchText.toLowerCase()) ||
    homeopathy.address.toLowerCase().includes(searchText.toLowerCase())
  );

  const renderHomeopathyItem = ({ item }) => (
    <View style={styles.homeopathyCard}>
      <Text style={styles.homeopathyName}>{item.name}</Text>
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.headerTitle}>হোমিওপ্যাথি</Text>
        <TextInput
      ...
      <FlatList
        data={filteredHomeopathies}
        renderItem={renderHomeopathyItem}
