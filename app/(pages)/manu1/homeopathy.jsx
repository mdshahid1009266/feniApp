...
const PharmacyPage = () => {
  ...
  useEffect(() => {
    const fetchPharmacies = async () => {
      try {
        const response = await axios.get('https://api-upokar.onrender.com/getHomio');
        if (response.data && response.data.length > 0) {
          setPharmacies(response.data);
        } else {
          setError('No homeopathy care found');
        }
      } catch (err) {
        setError('Failed to fetch homeopathy care. Please try again later.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleWhatsApp = (phoneNumber) => {
    ...
    Linking.openURL(`whatsapp://send?phone=${formattedNumber}&text=হ্যালো`);
  };
    ...
    if (filteredPharmacies.length === 0) {
      return (
        <View style={styles.centered}>
          ...
          <Text style={styles.noResultsText}>কোন হোমিওপ্যাথিপাওয়া যায়নি</Text>
        </View>
      );
    }
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <TextInput
          ...
          placeholder="হোমিওপ্যাথি খুঁজুন..."
