...
const ProductCard = ({ product, onPress }) => {
  return (
    <Link
      href={{
        pathname: "manu5/byegift",
        params: { gid: product.id }
      }}
...
const ProductsPage = () => {
  ...
  const { gitem } = useLocalSearchParams();
    ...
    <SafeAreaView className="flex-1 bg-blue-100">
      ...
      {filteredProducts.length === 0 ? (
        <View className="flex-1 justify-center items-center">
          <Text className="text-gray-500 text-lg font-[BanglaFont]">কোনো গিফট পাওয়া যায়নি</Text>
        </View>
      ) : (
        <FlatList
